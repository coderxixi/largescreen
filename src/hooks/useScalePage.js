...
export default function useScalePage() {
    ...
    function triggerScale(option = {}) {
        ...
        if (currentRatio > targetRatio) {
            ...
            // 3.设置缩放( 类似图片放大 )
            document.body.setAttribute('style', `width:${targetX}px;height:${targetY}px;transform: scale(${scaleRatio})  translateX(-50%); left:50%`);
        } else {
            // 3.设置缩放( 类似图片放大 )
            document.body.setAttribute('style', `width:${targetX}px;height:${targetY}px;transform: scale(${scaleRatio})`);
        }
    }
    onMounted(() => {
        triggerScale();
